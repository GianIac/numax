...
- [x] Explicit legacy migration version 0 → 1
- [x] Generic migration-step registry for sequential N → N+1 upgrades
- [x] Fixture-based migration test generated by the previous release
- [x] "Rolling upgrade" rejection test: previous release binary + current binary reject incompatible protocol versions without exchanging ops
