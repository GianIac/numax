...
impl Runtime {
    ...
    pub fn run_module(&self, wasm_bytes: &[u8]) -> Result<()> {
        // Costruisce lo stato host per questo run.
        let host_state = if self.config.enable_wasi {
            let wasi = WasiCtx::builder()
                .inherit_stdio()
                .inherit_args()
                .build_p1();
            HostState { wasi: Some(wasi) }
        } else {
            HostState { wasi: None }
        };
        ...
        // 1. Compilazione / validazione modulo
        let module = Module::new(&self.engine, wasm_bytes)
            .map_err(|_e| anyhow!("Invalid module (failed to compile/validate)"))?;

        // 2. Istanziazione (linking import/host)
        let instance = self
            .linker
            .instantiate(&mut store, &module)
            .map_err(|_e| anyhow!(
                "Link error while instantiating module (missing import or incompatible types)"
            ))?;

        // 3. Entrypoint: prima `run`, poi `_start`
        let run = instance
