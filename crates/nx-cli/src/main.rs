...
async fn real_main() -> Result<()> {
    ...
    let matches = cli_command(color_choice).get_matches();
    let cli =
        Cli::from_arg_matches(&matches).expect("clap matches should convert to CLI arguments");
